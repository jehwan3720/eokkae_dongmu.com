    ...
    <section className="bg-[#F8F9FB] py-24 md:py-32 border-t border-[#E8EAED]">
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20">
        ...
        <motion.div
          ...
          viewport={VIEWPORT}
        >
          {documents.map(({ Icon, title, desc, tag }, i) => (
            <motion.div
              key={title}
              variants={slideUpStagger}
              custom={i}
              className="group bg-white border border-[#E8EAED] rounded-2xl p-7 flex flex-col gap-5 hover:-translate-y-1 hover:shadow-[0_8px_32px_rgba(15,31,61,0.10)] transition-all duration-300 cursor-default"
            >
              {/* 아이콘 */}
